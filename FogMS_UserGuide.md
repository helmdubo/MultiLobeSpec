...
1. Box читает плотность из вашей Volume Texture и собирает свет: солнце и локальные источники с тенями через
   аппаратный ray tracing, небо из кубмапы SkyLight, отражённый от стен свет из Lumen surface cache.
...
3. Поле J записывается в объёмную текстуру Box (`FogMS_TransportField`, 32³, FloatRGBA).
4. Volume-материал Box (`M_FogMS_Density`) читает J и отдаёт его в туман как Emissive = σs·J (σs — рассеивающая
   плотность, т. е. плотность × альбедо).
5. Штатный Volumetric Fog вокселизирует материал и интегрирует его к камере как обычно: свои фроксели,
   джиттер, временная история.

Итог: туман остаётся штатным, FogMS лишь подставляет ему правильно рассчитанную «подсветку изнутри».

**Гибрид (`Hybrid Single Scattering`):** делит работу иначе. Прямое солнце в Box рендерит штатный туман:
на разрешении фрокселей, с тенями от геометрии. Поле J несёт всё остальное: небо, локальные источники
и многократное рассеяние. Чтобы штатное солнце не светило сквозь толщу облака, материал затемняет его
на каждую ячейку 32³ множителем T_sun·k. T_sun — пропускание среды к солнцу, k — доля солнца в прямом свете ячейки.
Ночью k → 0, и гибрид превращается в обычную инъекцию.
...
- `FogMS_TransportField`: текстура поля. Её uvw = (Local + Extent) / (2·Extent) в осях Box.
  Alpha 0 означает, что поля нет, и материал возвращается к штатному освещению по альбедо; alpha ≥ 0,5 означает,
  что поле валидно.

Материал должен считать так: valid = (A ≥ 0,5), Emissive = valid·RGB·Albedo·σt. Режим 1: BaseColor = Albedo·(1 − valid).
Режим 2: BaseColor = Albedo·lerp(1, saturate(2A − 1), valid). Как устроен сам `M_FogMS_Density`, (не проверено):
.uasset не читается, контракт взят из кода Box.
...
| **UE 5.8.2 точно** | Источник Lumen собран только под 5.8.2 («Lumen source adapter is supported only on UE 5.8.2…»). Если он не работает, решатель отказывает целиком (`FogMS_Fab_Readiness.md` §0 п. 3). В `.uplugin` стоит `EngineVersion 5.8.0` |
...
| `r.RayTracing.Nanite.Mode 0` (так по умолчанию) | Этого требует источник Lumen |
| Exponential Height Fog с Volumetric Fog, **Scattering Distribution = 0** | Иначе «Spatial scattering requires fog Scattering Distribution=0.» |
...
(по умолчанию вкл.) в `BeginPlay` сам ставит `r.RayTracing.Culling 0` и `r.Lumen.AsyncCompute 0`. Он делает это
с низким приоритетом `SetByGameSetting`: если значение уже задано в ini проекта, в device profile или в командной
строке, оно сохраняется, а в лог пишется «kept… Transport stays off». В PIE и в редакторе эта опция не работает.
Там нажмите кнопку **Enable Indirect Preview** или пропишите оба cvar в `DefaultEngine.ini`. Учтите, что кнопка
меняет и настройки Lumen Translucency Volume до конца сессии; вернуть их можно кнопкой **Restore Standard Lumen**.

**Что требует `-BindlessAll` (только редактор, «Advanced»).** В упакованной игре ключ `-BindlessAll` не читается:
он обрабатывается только в редакторной сборке. Поэтому единственный рабочий путь в игре — Transport + Emissive
Injection. Без `-BindlessAll` недоступны overlay-режимы: доставка B2/B3 через overlay, Octaves, `Spatial (Experimental)`,
`World (Current Frame)`, A1d/A1e (`Authored Sun Shadow`, `Cast Sun Shadow`, фильтрованный shadow cache), `r.FogMS.ViewIntegration`,
SSFS sky disk, отладочные виды, `r.FogMS.BoxMode 1`. Box в этом случае показывает статус вида «…requires -BindlessAll
(injection-only: no BindlessAll; overlay features off)» и освещает туман штатно.
...
6. По желанию включите `Hybrid Single Scattering`.
7. Анимация: `World Aligned Texture` = вкл. (без него анимация не работает, статус «Static: animation requires World
...
8. Посмотрите `Spatial Status`. Рабочее состояние выглядит так:
   `Active B3 isotropic transport (16 directions, tol 1.00e-06; emissive injection via material; hybrid: native single
   scattering; see convergence diagnostics) (injection-only: no BindlessAll; overlay features off)`.
   С async-решателем добавится `; one frame late`, с интервалом решения — `; hold k/N` или `; solve every N frames`.
...
| `Scattering Mode` | Алгоритм. Для продукта нужен `Transport (B3 Angular)`. `Transport (B2)` — 6 осевых направлений | B2 ≤8 итераций ≈ 1,04 мс, но J ярче эталона на ~11 % (ошибка 16 %). Это кандидат в «Economy», в пресеты не входит |
...
| `Emissive Injection` | Доставляет J через Volume-материал Box (раздел 1) | На ~0,7 мс дешевле overlay в `LightScattering` (1,575 → 0,870 мс). Это единственный путь без `-BindlessAll` |
...
(размер одного повтора текстуры в см), `Texture Offset (World)`, `Tile Scale` (только без World Aligned), `Threshold`/`Softness`
(порог шума и ширина перехода), `Detail Strength`/`Detail Scale`/`Detail Second Octave` (детали из той же текстуры),
`Density`, `Density Albedo`. Любая правка сразу меняет картинку.

**Анимация (FogMS|Density Animation):** `Animate Density`, `Wind Speed`, `Edge Flow Speed`. Кнопки
`FreezeDensityAnimation`/`ResumeDensityAnimation` замораживают и возобновляют фазу, `ResetMotionOrigin` сбрасывает её.
`Use Manual Animation Time` + `Manual Animation Time` дают воспроизводимый кадр.

**Солнечные тени A1d/A1e (FogMS|Sun)** — `Authored Sun Shadow`, `Cast Sun Shadow`, `Filtered Sun Shadow` —
и **FogMS|Indirect** (`Indirect Shadowing`) работают только с overlay (`-BindlessAll`, редактор). Transport не использует
`Indirect Shadowing`, `Spatial Strength` и `Spatial Distance`.

**Интервал решения** задаётся не свойством Box, а cvar `r.FogMS.Transport.SolveInterval` (раздел 5).
...
| `r.FogMS.World.SunExcludeDegrees` | 3 | Вырезает солнечный диск из кубмапы неба, чтобы не считать солнце дважды |
| `r.FogMS.World.SkyMipBias` | 0 | Больше — небо размытее, меньше — резче |
| `r.FogMS.World.Indirect` | 1 | Диагностика: 0 — только прямые источники, без неба и Lumen |

**Доставка, async, удержание:**

| Cvar | По умолчанию | Когда трогать |
|---|---|---|
| `r.FogMS.Transport.AsyncCompute` | 0 | Ставьте 1 вместе с Emissive Injection: решатель уходит в async-очередь, а J приходит в туман на кадр позже. Нужны `r.RDG.AsyncCompute` > 0 и поддержка async в RHI, иначе проходы тихо остаются на графике |
| `r.FogMS.Transport.SolveInterval` | 1 | Решать раз в N кадров (1…8), между решениями держать прошлый результат. В отчёте рекомендовано 2 для основного Box и 4 для дальних Box. Любое изменение Box, настроек или солнца запускает пересчёт сразу |
...
- **Инъекция вместо overlay:** штатный `LightScattering` дешевле на ~0,7 мс (1,575 → 0,870 мс). Кадр в целом —
  11,55 → 10,76 мс.
- **Async (`AsyncCompute 1`) + инъекция:** на графической очереди остаётся ~0,35–0,46 мс (проходы с ray tracing).
  Кадр дешевле на ~1,07 мс из 2,2 мс решателя, но `ComputeVolumetricFog` дорожает (3,46 → 3,81 мс), потому что
  делит GPU с async-очередью. Временные буферы занимают ~25 МБ при 48 направлениях и ~50 МБ при 96.
- **`SolveInterval 2`:** на кадрах удержания решатель стоит 0,00 мс. Картинка против N = 1 — 46 дБ при N = 2
  и 44 дБ при N = 4.
- **Гибрид:** стоит столько же, сколько полная инъекция (+0,02 мс).
- **B2 «Economy»:** ≈ 1,04 мс при ≤8 итерациях, J ярче эталона на ~11 %.
- **Штатный туман** при 4 px / 208 слоях стоит ещё ~5–6 мс. Отчёт предполагает для продакшена 8–16 px.

**Что влияет на цену:** число направлений (линейно), итерации и tolerance (адаптивно: решатель останавливается,
как только сошёлся), интервал решения, async. Размер сетки фиксирован (32³). Как цена зависит от размера Box
на экране и от числа источников, (не проверено).
...
- **Один активный Box на мир.** Если включённых и видимых Box два, эффект отключается («More than one enabled,
  visible FogMS Box…»).
- **Ночь при статическом SkyLight.** Кубмапа статического SkyLight после заката остаётся дневной, и небо продолжает
  подсвечивать Box (как и штатный туман). Для смены дня и ночи включите у SkyLight Real Time Capture.
  С ним в сумерках Box отличается от штатного освещения на 5–7 %.
- **Задержка.** При `AsyncCompute 1` поле приходит на кадр позже. При `SolveInterval N` изменения, которые не
  запускают пересчёт (локальные источники, небо, Lumen, фаза анимации плотности), приходят с задержкой до N−1 кадров.
  Движение солнца пересчитывается каждый кадр. Кроме того, при инъекции J проходит через штатную временную историю
  тумана, поэтому отклик на смену света сглажен.
- **Лампа внутри облака при гибриде.** Штатная часть однократного рассеяния от лампы тоже умножается на T_sun·k;
  остальной её свет идёт через поле 32³, без теней от геометрии на разрешении фрокселей. Как это выглядит,
  (не проверено).
...
- **Движение Box:** по замыслу, пока Box перетаскивают, он откатывается к штатному освещению, и история тумана сбрасывается (не проверено).
- **Статус «Requires…» / «…requires…»** — не выполнено требование из раздела 2. Нужный cvar или настройка
  указаны в тексте.
- **«Transport needs Emissive Injection or -BindlessAll…»** — включите `Emissive Injection`.
- **«Waiting for current-frame isotropic transport»** — решатель ещё не опубликовал поле. Если статус не
  меняется, проверьте, что Box виден в вьюпорте, а вьюпорт в режиме realtime (не проверено).
...
- **«Transport unavailable; native lighting with authored density: …»** — решатель отказал, причина указана после
  двоеточия. Часто это неподдерживаемый источник или источник Lumen («Lumen source waiting…» бывает в первые кадры).
...
`Tools/FogMSEnergyValidation/ProdProbe/` содержит скрипты Python и shell. Они управляют запущенным редактором через мост
UE-MCP (`ws://127.0.0.1:9877`) и нужны тем, кто хочет повторить замеры из отчёта. Нужны Python 3, numpy и Pillow.
Во время замера редактор должен быть окном на переднем плане.

- `measure.py`, `gpuprofile.py` — выставляют настройки Box, запускают `ProfileGPU`, `FogMS.DumpSpatial` и делают скриншот;
- `tierab.sh`, `abinject.sh`, `hybridab.sh`, `intervalab.sh`, `asyncab.sh` — A/B тиров, доставки, гибрида, интервала и async;
- `compare.py`, `fielddiff.py`, `resid_stats.py` — PSNR скриншотов, разница полей, невязка решателя;
- `nightcmp.py`, `soak.py`, `nobindless_test.sh`, `pie_test.py` — сумерки, длительный прогон, запуск без `-BindlessAll`, PIE;
- `gen_perlin_worley.py` — генератор бесшовного 3D-шума для Volume Texture.
