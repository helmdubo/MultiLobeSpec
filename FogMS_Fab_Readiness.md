...
> **Статус 2026-09-23 (поздний вечер), раунды 21–27.** P5: публичные флаги теней — единственный источник (диагностика раунда 21
> доказала тождественность, приватный буфер и `HitFlagsDebug` удалены). P9: небо — `r.FogMS.World.SkySource` авто (Sky View LUT
> при Real Time Capture, публичный захват `GetProcessedSkyTexture` иначе, SH как запасной); приватный путь удалён; со
> статическим захватом публичный путь тождествен (раунд 25). P8: `Lumen Bounce` Auto/Off — Lumen-кэш остаётся основным
> (проверенная сборка 5.8.2), при его отсутствии публичный откат (альбедо × (солнце с тенью + SH-небо)), в работе «откат v2»
> (ослабление средой, альбедо земли на Box). P10, P2–P7 — сделано. **Остаются приватными:** список источников света P11
> (`FScene::Lights`, `AtmosphereLights`, `VolumetricCloud` через `ScenePrivate.h`/`LightSceneInfo.h`) и сам Lumen-источник
> (один файл, за опцией, с заглушками для других версий). SSFS-постфильтр и Spatial — Advanced.
