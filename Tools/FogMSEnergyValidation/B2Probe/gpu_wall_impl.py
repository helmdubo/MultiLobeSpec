...
"""Explicit B2 thin-wall probe; reuses one existing cube, never creates assets.
...
def default_cases():
    ...
    for position in ('center', 'quarter_cell', 'boundary_half_cell'):
        for shown in (False, True):
            result.append(dict(name=('wall-' if shown else 'no-wall-')+position,
                               test=1, tau=4, albedo=.9, boundary=1, geometry=1,
                               wall_position=position, wall_shown=shown))
    return result
...
def launch(config_name='gpu_wall_config.json'):
    import unreal
    config = json.loads((ROOT / config_name).read_text(encoding='utf-8-sig'))
    reconstruction_test = bool(config.get('reconstruction_test', False))
    ...
    elif reconstruction_test:
        raise RuntimeError('Reconstruction diagnostic needs the Package6 CVar/shader; standard capture is not a fallback.')
    ...
    numerical = sys.modules.get('_fogms_b2_numerical_probe')
    if numerical and getattr(numerical,'controller',None) and not numerical.controller.restoration_ok:
        raise RuntimeError('Finish/restore the numerical probe before running the wall probe.')
    if action == 'stop':
    ...
    def restore(original):
        ...
        return {'ok': not errors and len(checks) == 8 and all(checks.values()), 'checks': checks, 'errors': errors}

    if action == 'restore':
        name = config.get('restore_receipt', '')
        if pathlib.Path(name).name != name or not name.startswith('gpu_wall-') or not name.endswith('.json'):
            raise ValueError('restore_receipt must name a local gpu_wall-*.json produced by this harness.')
        path = ROOT / name
        ...
        unreal.log('FOGMS_B2_WALL_RESTORED ' + str(path))
        return
    ...
    receipt = ROOT / ('gpu_wall-' + case_id + '.json')
    if receipt.exists():
    ...
    class Probe:
        ...
        def write(self, status):
            atomic_json(receipt, {
                'schema': 'fogms_b2_wall_probe_v1', 'id': case_id, 'status': status,
                ...
                'note': 'One existing cube, six matched no-wall/wall captures; readback stalls are not GPU timing.'
            })
        ...
        def start(self):
            ...
            self.write('RUNNING')
            unreal.log('FOGMS_B2_WALL_ACCEPTED ' + str(receipt) + ' (asynchronous; wait for COMPLETED)')
        ...
        def finish(self, status, reason):
            ...
            try:
                ...
                self.write(status)
                unreal.log('FOGMS_B2_WALL_FINISHED ' + status + ' ' + str(receipt))
