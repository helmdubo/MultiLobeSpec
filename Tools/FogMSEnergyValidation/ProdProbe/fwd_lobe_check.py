# -*- coding: utf-8 -*-
"""W37 CPU proof, no editor. Usage: python fwd_lobe_check.py   (numpy; exit code 0 = PASS)

F1a forward lobe. Evaluates the material HLSL itself: FORWARD_LOBE_CODE_V1 is read from matedit_density.py as text (ast, no
`import unreal`) and its statements are translated line by line to numpy (translate()); an independent float64 reference
...
and cross-checked on real 3D vectors (Fibonacci sphere of view directions, random sun directions). Checks for
g in {0, 0.3, 0.6, 0.8, 0.9} x Strength x S x Depth x Floor:
  1. translation == reference (float64 1e-12 relative, the HLSL in float32 1e-4);
  2. |mean - 1| <= 1e-3 (float64 and float32 HLSL; the math gives 1 exactly: each octave phase is F + (1 - F) * 4pi*HG);
...
def hlsl_code():
    ...
    for node in tree.body:
        if isinstance(node, ast.Assign) and any(getattr(t, "id", "") == "FORWARD_LOBE_CODE_V1" for t in node.targets):
            return node.value.value
    raise SystemExit("FORWARD_LOBE_CODE_V1 not found in " + MATEDIT)

def translate(code):
    """(condition, [statements]) as Python source. Accepts only the restricted form of FORWARD_LOBE_CODE_V1: one statement
    ...
    if lines[:2] != head or lines[3] != "{" or lines[-2] != "}" or lines[-1] != "return Result;":
        raise SystemExit("FORWARD_LOBE_CODE_V1 layout changed; update translate(): %r" % lines)
...
def run_hlsl(prog, Strength, G, Depth, BackFloor, FieldA, Mode, SunDir, CameraVector, dtype=np.float64):
    ...
    env = dict(NS, Strength=f(Strength), G=f(G), Depth=f(Depth), BackFloor=f(BackFloor), FieldA=f(FieldA), Mode=f(Mode),
               SunDir=f(SunDir), CameraVector=f(CameraVector), Emissive=f(1.0))
...
def reference(mu, s, g, b, F, S):
    """The math, float64: 1 + (1 - F) * (f1 (p(g) - 1) + f2 (p(g/2) - 1))."""
    p = lambda gg: (1 - gg * gg) / (1 + gg * gg - 2 * gg * mu) ** 1.5
    f1 = s * 2 / 3 * S ** b if S > 0 else 0.0
    f2 = s * 1 / 3 * S ** (b * b) if S > 0 else 0.0
    return 1 + (1 - F) * (f1 * (p(g) - 1) + f2 * (p(0.5 * g) - 1))
...
def lobe_checks():
    prog = translate(hlsl_code())
    print("HLSL condition:", prog[0]); print("HLSL body:\n  " + "\n  ".join(prog[1]))
    CAM = cam_from_mu(MU); SUN = np.array([0.0, 0.0, 1.0])
    GRID = np.concatenate([np.linspace(-1, 1, 200001), [-1.0, 1.0]]); CAMG = cam_from_mu(GRID)
    worst = {"tr64": 0.0, "tr32": 0.0, "mean64": 0.0, "mean32": 0.0, "minmargin": 1e9, "n": 0}
            ...
            for S in (0.0, 0.05, 0.5, 1.0):
                for b in (0.05, 0.5, 1.0):
                    for F in (0.0, 0.25, 0.5, 1.0):
                        A = 0.5 + 0.5 * S
                        ref = reference(MU, s, g, b, F, S)
                        h64, taken = run_hlsl(prog, s, g, b, F, A, 2.0, SUN, CAM)
                        h32, _ = run_hlsl(prog, s, g, b, F, A, 2.0, SUN, CAM, np.float32)
                        ...
                        lo, _ = run_hlsl(prog, s, g, b, F, A, 2.0, SUN, CAMG, np.float32)
                        worst["minmargin"] = min(worst["minmargin"], float(lo.min()) - (1 - s * (1 - F)))
                        if S == 0.0 and not np.all(h32 == 1.0): check(False, "S = 0 must give Lobe 1 exactly (g %g s %g)" % (g, s))
                        worst["n"] += 1
    check(worst["tr64"] <= 1e-12, "translated HLSL == reference, float64 max rel %.2e (%d configs)" % (worst["tr64"], worst["n"]))
    # float32: h = 1 + g^2 - 2 g mu cancels to (1 - g)^2 = 0.01 at g 0.9, mu 1: ~1e-7 * 2 / 0.01 relative, x1.5 in h^-1.5.
    check(worst["tr32"] <= 1e-4, "translated HLSL in float32 vs reference max rel %.2e (cancellation in 1+g^2-2g mu)" % worst["tr32"])
    check(worst["mean64"] <= 1e-3, "mean over the sphere = 1: max |mean-1| %.2e (float64)" % worst["mean64"])
    check(worst["mean32"] <= 1e-3, "mean over the sphere = 1: max |mean-1| %.2e (HLSL in float32)" % worst["mean32"])
    ...
    V = fibonacci(400000); rng = np.random.default_rng(37); worst3 = 0.0
    for trial in range(3):
        L = rng.normal(size=3); L /= np.linalg.norm(L)
        for g in (0.0, 0.3, 0.6, 0.8, 0.9):
            r, _ = run_hlsl(prog, 0.8, g, 0.5, 0.25, 1.0, 2.0, L * 0.999, -V, np.float32)  # unnormalized sun on purpose
            worst3 = max(worst3, abs(float(r.mean()) - 1))
    check(worst3 <= 1e-3, "3D Fibonacci sphere (400k view dirs, 3 random suns, s 0.8, F 0.25): max |mean-1| %.2e" % worst3)
    ...
    for s, mode in ((0.0, 2.0), (0.8, 1.0), (0.8, 3.0), (0.8, 0.0)):
        r, taken = run_hlsl(prog, s, 0.6, 0.5, 0.25, 1.0, mode, SUN, CAM, np.float32)
    ...
    print("\nLobe (x isotropic) at S = T_sun*k, Depth 0.5, Floor 0.25:")
    print("  %-14s %6s | %8s %8s %8s %8s" % ("config", "S", "at sun", "15 deg", "side", "behind"))
    mus = np.array([1.0, math.cos(math.radians(15)), 0.0, -1.0])
    for s, g in ((0.5, 0.6), (0.8, 0.6), (0.8, 0.8)):
        for S in (1.0, 0.5, 0.1):
            v = reference(mus, s, g, 0.5, 0.25, S)
            print("  s %.1f g %.1f    %6.2f | %8.2f %8.2f %8.2f %8.2f" % (s, g, S, *v))
